test_that("fit 3PL model on generated data", {
  ...
  expect_warning(fit8b <- difNLR(Data[sam, ], group[sam], focal.name = 1, model = "4PL",
...
test_that("fit different models on generated data", {
  ...
  fit4 <- difNLR(Data, group, focal.name = 1, model = model,
                 constraints = constraints, type = type)
  expect_s3_class(fit4, "difNLR")
  expect_equal(fit4$DIFitems, c(5, 8, 11, 15))

  # item purification
  fit9 <- difNLR(Data[, 1:6], group, focal.name = 1, match = "score",
  ...
  # anchor items
  fit9b <- difNLR(Data[, 1:6], group, focal.name = 1, match = "score",
  ...
  fit10 <- difNLR(Data[, 1:6], group, focal.name = 1, match = "score",
  ...
  fit11 <- difNLR(Data[, 1:6], group, focal.name = 1, match = "score",
                  model = "4PL", type = "all", p.adjust.method = "holm",
                  purify = TRUE)
