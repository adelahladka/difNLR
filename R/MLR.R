...
MLR <- function(Data, group, key, type = "both", match = "zscore", anchor = 1:ncol(Data), p.adjust.method = "none",
  ...
  if (parametrization == "irt"){
    ...
    par.m0.delta = lapply(par.m0.tmp, function(x) matrix(cbind(-x[, "(Intercept)"]/x[, "x"],
                                                               x[, "x"],
                                                               (x[, "(Intercept)"]*x[, "x:group"] - x[, "x"]*x[, "group"])/(x[, "x"]^2 + x[, "x"]*x[, "x:group"]),
                                                               x[, "x:group"]),
                                                         ncol = 4,
                                                         dimnames = list(rownames(x), c("b", "a", "bDIF", "aDIF"))))
    par.m1.delta = lapply(par.m1.tmp, function(x) matrix(cbind(b = -x[, "(Intercept)"]/x[, "x"],
                                                               a = x[, "x"],
    ...
    cov.m0.tmp = lapply(1:m, function(i)
      rbind(cbind(cov.m0[[i]], matrix(0, nrow = nrow(cov.m0[[i]]), ncol = (4*nrow(par.m0[[i]]) - ncol(cov.m0[[i]])))),
    ...
    cov.m1.tmp = lapply(1:m, function(i)
      rbind(cbind(cov.m1[[i]], matrix(0, nrow = nrow(cov.m1[[i]]), ncol = (4*nrow(par.m1[[i]]) - ncol(cov.m1[[i]])))),
    ...
    se.m0 = lapply(1:m, function(i) {
      if (length(cat.m0[[i]]) > 1) {
        cbind(sapply(cat.m0[[i]], function(j) deltamethod(~ -x1/x2,
                                                          par.m0.tmp[[i]][j, c("(Intercept)", "x")],
                                                          cov.m0.tmp[[i]][paste(j, c("(Intercept)", "x"), sep = ":"),
                                                                          paste(j, c("(Intercept)", "x"), sep = ":")])),
              sqrt(diag(cov.m0.tmp[[i]][paste(cat.m0[[i]], "x", sep = ":"), paste(cat.m0[[i]], "x", sep = ":")])),
              sapply(cat.m0[[i]], function(j) deltamethod(~ (x1*x4 - x2*x3)/(x2*x2 + x2*x4),
                                                          par.m0.tmp[[i]][j, ],
        ...
        matrix(c(sapply(cat.m0[[i]], function(j) deltamethod(~ -x1/x2,
                                                             par.m0.tmp[[i]][j, c("(Intercept)", "x")],
                                                             cov.m0.tmp[[i]][paste(j, c("(Intercept)", "x"), sep = ":"),
                                                                             paste(j, c("(Intercept)", "x"), sep = ":")])),
    ...
    se.m1 = lapply(1:m, function(i) {
      if (length(cat.m0[[i]]) > 1) {
        cbind(sapply(cat.m1[[i]], function(j) deltamethod(~ -x1/x2,
                                                          par.m1.tmp[[i]][j, c("(Intercept)", "x")],
                                                          cov.m1.tmp[[i]][paste(j, c("(Intercept)", "x"), sep = ":"),
                                                                          paste(j, c("(Intercept)", "x"), sep = ":")])),
              sqrt(diag(cov.m1.tmp[[i]][paste(cat.m1[[i]], "x", sep = ":"), paste(cat.m1[[i]], "x", sep = ":")])),
              sapply(cat.m1[[i]], function(j) deltamethod(~ (x1*x4 - x2*x3)/(x2*x2 + x2*x4),
                                                          par.m1.tmp[[i]][j, ],
        ...
        matrix(c(sapply(cat.m1[[i]], function(j) deltamethod(~ -x1/x2,
                                                             par.m1.tmp[[i]][j, c("(Intercept)", "x")],
                                                             cov.m1.tmp[[i]][paste(j, c("(Intercept)", "x"), sep = ":"),
                                                                             paste(j, c("(Intercept)", "x"), sep = ":")])),
